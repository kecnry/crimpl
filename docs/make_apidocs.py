...
if __name__ == '__main__':
    ...
    print("CREATING API DOCS FOR crimpl VERSION: {}".format(crimpl.__version__))

    fms = api_docs(crimpl, skip=[], members=[pydoc.inspect.isfunction])

    skip_methods = []
    fms = api_docs(crimpl.LocalThreadServer, skip=skip_methods)
    fms = api_docs(crimpl.LocalThreadJob, skip=skip_methods)
    fms = api_docs(crimpl.RemoteThreadServer, skip=skip_methods)
    ...
    fms = api_docs(crimpl.RemotePBSServer, skip=skip_methods)
    fms = api_docs(crimpl.RemotePBSJob, skip=skip_methods)
